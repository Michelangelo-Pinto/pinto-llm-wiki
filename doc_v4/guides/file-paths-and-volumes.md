...
Where files go for ingestion and how to move files between your host machine and the Docker containers.

## Volume Map

| Volume Name | Mount Inside Container | Purpose | Container | Access |
|-------------|----------------------|---------|-----------|--------|
| `shared_data` / `to_ingest/` | `/data/shared` | Files to ingest (PDF, DOCX, images, MD, HTML) | `ingestion-pipeline`, `tesseract-mcp` | Read-only (`:ro`) |
...
```bash
docker compose down -v   # Removes ALL data including Qdrant vectors, ingestion SQLite
```

**To reset only ingestion state (keep Qdrant):**
